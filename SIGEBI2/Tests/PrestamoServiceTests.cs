...
namespace Tests
{
    ...
    public class PrestamoServiceTests
    {
        ...
        public void Setup()
        {
            ...
            // Limpiar tablas antes de iniciar (orden correcto según FK)
        ...
        [TestMethod]
        public async Task RegistrarPrestamo_DeberiaCrearPrestamoYDetalles()
        {
            var usuario = new Usuario
            {
                Nombre = "Usuario Test",
                Email = $"test{Guid.NewGuid():N}@mail.com", // evitar duplicados
                Contraseña = "123",
                Rol = "Estudiante",
                Activo = true
            };
            ...
            var libro = new Libro
            {
                ...
                ISBN = "978" + Guid.NewGuid().ToString("N").Substring(0, 10), // evitar conflicto UNIQUE
            ...
            Assert.IsTrue(result.Success, $"❌ Falló el registro de préstamo: {result.Message}");
            Assert.IsNotNull(result.Data, "El préstamo devuelto es nulo.");
            Assert.AreEqual(usuario.Id, result.Data.UsuarioId, "El préstamo no está asociado al usuario correcto.");

            var actualizado = await _context.Libros.FindAsync(libro.Id);
            Assert.IsFalse(actualizado!.Disponible, "El libro debería haber sido marcado como no disponible.");
        }

        // CU-10: Registrar devolución
        [TestMethod]
        public async Task RegistrarDevolucion_DeberiaActualizarLibroYMarcarPrestamoCerrado()
        {
            var usuario = new Usuario
            {
                Nombre = "Carlos",
            ...
            Assert.IsFalse(prestamoCerrado!.Activo, "El préstamo debería haberse marcado como cerrado.");
        }

        // CU-11: Consultar préstamos activos por usuario
        [TestMethod]
        public async Task ObtenerPrestamosActivosPorUsuario_DeberiaRetornarLista()
        {
            var usuario = new Usuario
            {
                Nombre = "Ana",
            ...
            Assert.IsTrue(result.Success, "No se pudieron obtener los préstamos activos.");
            Assert.IsTrue(result.Data!.Any(), "La lista de préstamos activos está vacía.");
        }

        // CU-12: Consultar préstamos vencidos
        [TestMethod]
        public async Task ObtenerPrestamosVencidos_DeberiaRetornarPrestamosConFechaVencida()
        {
            var usuario = new Usuario
            {
                Nombre = "Luis",
            ...
            Assert.IsTrue(result.Success || (!result.Success && result.Message!.Contains("vencidos")),
                "El método ObtenerPrestamosVencidosAsync no devolvió el resultado esperado.");
        }
    }
}
